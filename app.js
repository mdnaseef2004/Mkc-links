...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Card tap delegation for mobile native app experience
    const linkCards = document.querySelectorAll('.link-card');
    linkCards.forEach(card => {
        card.addEventListener('click', (e) => {
            // Prevent duplicate trigger if direct link/button was clicked
            if (e.target.closest('.copy-btn') || e.target.closest('.action-btn')) return;
    ...
    const applyTheme = (theme, isUserOverride = false) => {
        ...
        // Sync iOS Safari status bar theme-color
        const metaTheme = document.querySelector('meta[name="theme-color"]');
        if (metaTheme) {
            metaTheme.setAttribute('content', theme === 'light' ? '#F4F7FA' : '#0B111E');
        }
